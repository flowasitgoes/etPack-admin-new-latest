...
export default function PrintingStandaloneSidebar() {
  const [activeDepartment, setActiveDepartment] = useState("printing")
  const router = useRouter()
  const pathname = usePathname()

  // 根据当前路径确定active状态
  const getActiveModule = () => {
    if (pathname === '/printing/order-record') return 'order-record'
    if (pathname === '/printing/material-record') return 'material-record'
    if (pathname === '/printing/daily-report') return 'daily-report'
  ...
  const handleOrderClick = (order: RecentOrder) => {
    ...
    if (order.orderNumber === "K01140414001") {
      window.open(`http://localhost:3004/printing/${order.orderNumber}`, '_blank')
    ...
    <div className="admin-left-sidebar bg-gray-50 flex flex-col h-full">
      ...
      <nav className="space-y-2 px-4">
        ...
        <div 
          ...
          onClick={() => router.push('/printing/order-record')}
        >
          訂製單記錄
        </div>
        <div 
          ...
          onClick={() => router.push('/printing/material-record')}
        >
          領料記錄
        </div>
        <div 
          ...
          onClick={() => router.push('/printing/daily-report')}
        >
          生產/檢驗日報表
        </div>
        <div 
          ...
          onClick={() => router.push('/printing/recipe-database')}
        >
          配方資料庫
        </div>
        <div 
