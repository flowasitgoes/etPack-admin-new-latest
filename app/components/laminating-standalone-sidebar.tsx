...
export default function LaminatingStandaloneSidebar() {
  const [activeDepartment, setActiveDepartment] = useState("laminating")
  const router = useRouter()
  const pathname = usePathname()

  // 根据当前路径确定active状态
  const getActiveModule = () => {
    if (pathname === '/laminating/order-record') return 'order-record'
    if (pathname === '/laminating/material-record') return 'material-record'
    if (pathname === '/laminating/daily-report') return 'daily-report'
  ...
  const handleOrderClick = (order: RecentOrder) => {
    ...
    if (order.orderNumber === "K01140414001") {
      window.open(`http://localhost:3004/laminating/${order.orderNumber}`, '_blank')
    ...
    <div className="admin-left-sidebar bg-gray-50 flex flex-col h-full">
      ...
      <nav className="space-y-2 px-4">
        <div 
          ...
          onClick={() => router.push('/laminating')}
        >
          生產排程
        </div>
        <div 
          ...
          onClick={() => router.push('/laminating/order-record')}
        >
          訂製單記錄
        </div>
        <div 
        ...
        <div 
          ...
          onClick={() => router.push('/laminating/daily-report')}
        >
          生產/檢驗日報表
        </div>
        <div 
          ...
          onClick={() => router.push('/laminating/recipe-database')}
        >
          配方資料庫
        </div>
      </nav>
